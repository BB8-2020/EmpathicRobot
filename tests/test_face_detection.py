...
def test_crop_to_face():
    ...
    image = np.load("../tests/datafacedetection/stormface.npy")
    width = 746
    height = 746
...
def test_face_from_image():
    ...
    """
    output = face_from_image("../tests/datafacedetection/stormface.BMP")
    assert type(output) is bytes
...
def test_raises_exception_on_non_detected_face():
    ...
    with pytest.raises(Exception) as excinfo:
        face_from_image("../tests/datafacedetection/false_image.BMP")
