...
from src.facedetection.face_detection import crop_to_face, reshape_image, convert_to_bytes, face_from_image


def test_crop_to_face():
    """Test the crop_to_face function by checking if the output is the right shape."""
    image = np.load("src/tests/datafacedetection/stormface.npy")
    width = 746
    height = 746
...
def test_face_from_image():
    ...
    """
    output = face_from_image("src/tests/datafacedetection/stormface.BMP")
    assert type(output) is bytes
...
def test_raises_exception_on_non_detected_face():
    ...
    with pytest.raises(Exception) as excinfo:
        face_from_image("src/tests/datafacedetection/false_image.BMP")
